...
const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ className = "", text }) => {
  const { t } = useLanguage();
  ...
  return (
    <button
      onClick={handleClick}
      className={`whatsapp-button inline-flex items-center space-x-2 ${className}`}
    >
      <MessageCircle className="w-5 h-5" />
      <span>{text || t('home.whatsapp')}</span>
    </button>
  );
};
